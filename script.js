document.addEventListener("DOMContentLoaded", function () {
	...
	function createAddToCartButton() {
		...
		sizeSelector.innerHTML = [
			...
			'    <button type="button" class="size-option" data-size-option="Small" aria-pressed="false">Small</button>',
			'    <button type="button" class="size-option" data-size-option="Medium" aria-pressed="true">Medium</button>',
			'    <button type="button" class="size-option" data-size-option="Large" aria-pressed="false">Large</button>',
			'</div>'
		].join("");
	...
	function initializeCheckout() {
		...
		if (submitButton) {
			submitButton.addEventListener("click", function () {
				...
				isSubmitting = true;
				updateCheckoutStep();
				setCheckoutMessage("Placing your order and emailing the invoice...");

				sendInvoiceEmail(placedOrder).then(function (emailResult) {
					finalizeOrderPlacement(emailResult);
				});
			});
		}
		...
		function updateCheckoutStep() {
			...
			if (submitButton) {
				...
				submitButton.disabled = !canPlaceOrder || isSubmitting;
			}
		...
		function updatePaymentFields() {
			...
			var cardInputs = cardFields ? Array.prototype.slice.call(cardFields.querySelectorAll('input')) : [];
		...
		function sendInvoiceEmail(order) {
			if (!order || !order.formData || !order.formData.email) {
				return Promise.resolve({
					sent: false,
					error: "Order placed, but no billing email was provided for invoice delivery."
				});
			}

			return requestJson("/api/orders/invoice", {
				method: "POST",
				body: {
					order: order
				}
				...
				if (!response.ok) {
					return {
						sent: false,
						error: response.error || "Order placed, but we could not email the invoice."
					};
				}
				...
				return {
					sent: false,
					error: "Order placed, but the invoice email service is currently unavailable."
				};
			});
		}

		function finalizeOrderPlacement(emailResult) {
			var deliveryState = emailResult || {
				sent: false,
				error: "Order placed, but we could not email the invoice."
			};

			if (placedOrder) {
				placedOrder.invoiceEmail = {
					sent: Boolean(deliveryState.sent),
					sentAt: deliveryState.sent ? new Date().toISOString() : "",
					error: deliveryState.sent ? "" : deliveryState.error
				};
				persistLatestOrder(placedOrder);
			}
	...
	function initializeThankYouPage() {
		...
		if (invoiceStatus) {
			if (order.invoiceEmail && order.invoiceEmail.sent) {
				invoiceStatus.textContent = "Invoice emailed to " + (order.formData && order.formData.email ? order.formData.email : "your inbox") + ". You can preview and download it below.";
			} else if (order.invoiceEmail && order.invoiceEmail.error) {
				invoiceStatus.textContent = order.invoiceEmail.error + " You can still preview and download the invoice below.";
			} else {
	...
	function normalizeCartItem(entry) {
		...
		return {
			...
			image: entry && entry.image ? entry.image : getArtworkImagePath(entry),
	...
	function getArtworkImagePath(entry) {
		...
		return "gallerycontent/piece" + String(pieceNumber).padStart(2, "0") + ".jpg";
	}
